...
public class GameLogic : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        updateLayerGroups();
        liftHeight = checkRodHeight();
        ErrorTextBox = GameObject.FindGameObjectsWithTag("ErrorText")[0];
    ...
    void selectObject() //identify clicked object
    {
        ...
        if (Physics.Raycast(ray, out hit, 100.0f) && hit.transform != null)
            identifySelection(hit.transform.gameObject);
        else
        {
    ...
    void selectDestination(GameObject destination) //check destination disk for legal move
    {
        ...
        {
            errMessage("Cannot place above a smaller disk");
            return;
        }
    }
    ...
    float checkRodHeight() //check rod height for lifting
    {
        GameObject refRod = GameObject.FindGameObjectsWithTag("RodA")[0];
        return (refRod.transform.position.y + (checkSizeObj(refRod)) / 1.5f);
    }
